...
[override tags](/docs/ass-override-tags/); the styles exist so that you won't have to type out
...
: This controls the four text colors (primary, secondary, outline and shadow).
  ...
  **Secondary:**
  ...
  relatives on the [override tags page](/docs/ass-override-tags/#karaokeeffect/)).
  ...
  **Shadow**
  ...
  colors; clicking on them opens the [color picker](/docs/colour-picker/).
...
: Controls how close to the borders of the video frame the text will be
  ...
  vertical) is given in script resolution pixels (see [script properties](/docs/script-properties/)). For alignments (`\an`) 1-3, the vertical margin is
...
: Controls the outline (border) thickness (and style) and the shadow offset.
  ...
  - For the outline, the number is the thickness of the border in script pixels
    if ScaledBorderAndShadow is enabled (see [script properties](/docs/script-properties/));
...
: This section has various other parameters, namely:
  ...
  : Controls rotation of the text. The value is the angle of the rotation in
    ...
    when [animated transforms](/docs/ass-override-tags/#animatedtransform/) are involved).
