...
`@style` (`table`)
: A [style table](/docs/karaskel-lua/#style-table/) as defined by
...
Convert a path beginning with a [path specifier](/docs/path-specifiers/)
to an absolute path.

`@encoded_path` (`string`)
: A string which may optionally begin with an Aegisub [path specifier](/docs/path-specifiers/).

`@path` (`string`)
: If `encoded_path` began with a valid [path specifier](/docs/path-specifiers/), an absolute path. If it began with an
