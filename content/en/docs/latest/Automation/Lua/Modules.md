...
Some of these, such as [`karaskel.lua`](/docs/karaskel-lua/) implement subtitle-specific functionality, while others such as [re](/docs/re/) fill holes in the Lua standard library.

## Using modules

Simply write `modulename = require 'aegisub.modulename'` at the top level of a script.
For example, to include the [re](/docs/re/) modules, `re = require 'aegisub.re'`.
...
To use modules in [Karaoke Templater](/docs/karaoke-templater/), place the require statement in a [code once](/docs/code-lines-and-blocks/#classes-of-code-lines/) line.
Note that `karaskel.lua`, `utils.lua` and `unicode.lua` do not have to be `require`d in [Karaoke Templater](/docs/karaoke-templater/), as they are automatically imported.
...
[`cleantags.lua`](/docs/cleantags-lua/)
