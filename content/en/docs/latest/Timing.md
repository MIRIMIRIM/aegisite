...
[keyboard shortcuts](/docs/aegisub-options/#hotkeys/) associated with them by default):

1. Go to previous line, discarding any unsaved changes (previous syllable
   when in [karaoke mode](/docs/working-with-audio/#karaokemode/))
...
1. Add lead-in (how much is determined by the [audio lead in setting](/docs/aegisub-options/#audio/))
1. Add lead-out (exactly like the above, but the setting is called [audio lead out](/docs/aegisub-options/#audio/), logically enough)
...
loaded (see the [Working with video](/docs/working-with-video/) section), the white broken line
...
- You don't need to have video displayed while timing. If you think you do,
  ...
  manually, or with the [timing postprocessor](/docs/timing-post-processor/).
...
- If you want to delete all timing data and start over, a simple way is to use
  the [shift times dialog](/docs/shift-times/) to shift all lines backwards by
  9:59:59.99.
...
in the [audio options](/docs/working-with-audio/#options/). This is mostly important when you built
Aegisub yourself and did not use FFTW3; FFTW3 is fast enough that the default
quality is bumped up a bit.

## Karaoke timing

[Karaoke timing Tutorial](/docs/karaoke-timing-tutorial/)
