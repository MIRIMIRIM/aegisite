...
[排版](/zh-cn/docs/typesetting-introduction/)。关于字幕的时间轴编辑，另见
[载入音频](/zh-cn/docs/working-with-audio/)。
...
打开 [自动保存的字幕](/zh-cn/docs/autosave/)
...
1. 该行的 [样式](/zh-cn/docs/editing-styles/)。
1. 该行的说话人名称。这对于字幕本身并无影响，但能为编辑的过程提供便利。
1. 该行的特效。此处可以应用一些预置的特效，但由于渲染器的支持存在诸多缺陷，因此推荐使用
   [特效标签](/zh-cn/docs/ass-override-tags/)。这一项通常应用于自动化脚本的元数据区域。
1. 当前字幕中的最长行的字符数。
...
1. 打开 [颜色选择器](/zh-cn/docs/colour-picker/)
...
: 如果您在一个被检测为拼写错误的单词处(绿色波浪线标注)单击鼠标右键，词典就会向您提供一些近似的词汇，便于您进行修正。您也可以在菜单中设置拼写检查语言，或者向词典中添加无法正确识别的单词。若想了解更多有关信息，请参阅
  [拼写检查器](/zh-cn/docs/spell-checker/) 页面。
...
- 若想要改变 [特效标签](/zh-cn/docs/ass-override-tags/)
...
[字幕栏设置页面](/zh-cn/docs/aegisub-options/#General\_-.3E_Subtitles_grid/) 。
...
选择性粘贴...
: 打开 [选择性粘贴](/zh-cn/docs/paste-over/)
  对话框，可在其中进行多种操作。
