...
: An iterator which returns a table containing a [Match Table](/zh-cn/docs/re/#match-tables/) for the full match (if it matched), followed by a
  [Match Table](/zh-cn/docs/re/#match-tables/) for each capturing subexpression in the pattern
  (if any).
