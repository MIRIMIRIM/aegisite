...
Lua](/zh-cn/docs/lua-相关/)出现在Aegisub中、
...
另外一种特性是 **export
filter**(导出滤镜)。导出滤镜会被展示在[Export](/zh-cn/docs/导出/)
对话框中，在进行导出操作时被应用
...
这些全局变量会被展示在[Automation Manager](/zh-cn/docs/自动化脚本管理器/)
对话框中和脚本信息对话框中。
...
- **processing_function** (处理函数，函数) -
  ...
  processing function
  API](/zh-cn/docs/注册/#exportfilterprocessingfunction/)的要求。

- **configuration_panel_provider** (配置面板函数，函数，可选) -
  为导出滤镜提供了配置面板的函数
  如果不写这个函数，导出滤镜不会具有配置面板。需要遵循 [export filter
  configuration panel provider
  API](/zh-cn/docs/注册/#exportfilterconfigurationpanelprovider/) 的要求。
...
[`aegisub.register_macro`](/zh-cn/docs/注册/#aegisub.register_macro/)
must have this signature. The name `process_macro` is a placeholder for your
own function name.

- **subtitles** (user data) - The [subtitles object](/zh-cn/docs/字幕文件接口/) you use to manipulate
  the subtitles with.
...
- **subtitles** (user data) - The [subtitles object](/zh-cn/docs/字幕文件接口/) for the current subtitle
...
[`aegisub.register_filter`](/zh-cn/docs/注册/#aegisub.register_filter/)
...
- **settings** (table) - Configuration settings entered into the
  ...
  See the page on [configuration dialogues](/zh-cn/docs/对话框/) for more information
...
[configuration dialogues](/zh-cn/docs/对话框/) for
