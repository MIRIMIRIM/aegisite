...
建议您还是直接参考排版教程 或
[可视化排版](/zh-cn/docs/可视化排版/) .

## 打开视频

在 *视频_菜单中选择
\_打开视频*。你所能打开的视频文件类型取决于你的[视频提供器](/zh-cn/docs/选项/#video/)。如果你要使用空白视频，则点击
*打开空白视频*。
...
seeking，请尽量避免使用DSS。
注意，VFR视频不被Avisynth提供器支持，有时，载入[外部时间码](/zh-cn/docs/视频/#timecodes/)可能管用，但是实际上它会瞎搞一通。
Aegisub会在[?data](/zh-cn/docs/aegisub路径变量/#?data/)目录（Windows下通常在aegisub32.exe所在的文件夹）寻找Avisynth插件。你也可以直接把他们放在你的Avisynth插件文件夹来让它们自动载入。
...
[style](/zh-cn/docs/样式/) (or use the \\fscx or \\fscy
[overrides](/zh-cn/docs/ass标签/)) to the same amount but in the other
direction.
