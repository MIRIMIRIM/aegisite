---
title: 制作时间轴
slug: timing-subtitles-to-audio
menu:
  docs:
    parent: timing
weight: 5100
...
右边那几个能按下去的是切换按钮。按下去表示启用。按钮和控制按键的功能如下（许多默认都有[快捷键](/zh-cn/docs/keyboard-shortcuts/)）：

1. 移到上一行，放弃未提交的更改（在[卡拉OK模式](/zh-cn/docs/working-with-audio/#karaokemode/)时为上一音节）
1. 移到下一行，放弃未提交的更改（在[卡拉OK模式](/zh-cn/docs/working-with-audio/#karaokemode/)时为下一音节）
1. 播放所选波形的音频
1. 播放当前行
1. 停止播放
1. 播放所选部分前500ms
...
1. 开始时间提前（提前多少取决于[提前开始时间选项](/zh-cn/docs/aegisub-options/#audio/)）
1. 结束时间延后（和上一个意思差不多，取决于[延后结束时间选项](/zh-cn/docs/aegisub-options/#audio/)）
1. 提交（保存）更改
1. 将视图移至所选行的位置
...
当你在字幕栏中选中一行时，Aegisub将会在音频视图里高亮那行，如果你打开了自动卷动功能，音频视图会将所选行移至视图中间（一般打轴时不建议开启自动卷动）。你会注意到音频视图里会有各种竖线；粉色表示加载的视频（参阅[使用视频](/zh-cn/docs/working-with-video/)部分）的关键帧，白色分割线表示当前视频帧的位置,粗的红线和蓝线分别表示行的开始和结束，你可以左击或右击来设置开始或结束时间，或者拖动那些竖线。按
*播放* 按钮（默认快捷键为 *s*
...
- 在打轴时你不需要打开视频预览（译者注：仅当使用音轨打轴时）。如果你认为一定要看视频才能打轴，那么很可能是你理解有误。像一些配合视频场景打轴的情况，即让行的开始结束时间和场景的改变相同步，可以放到之后再手动或使用[时间后续处理器](/zh-cn/docs/timing-post-processor/)处理。
- 打开"提交后自动转至下一行"。
...
- 如果你想删除所有时间轴数据从头开始，有个简单的方法就是使用[平移时间窗口](/zh-cn/docs/shift-times/)来把所有行向后平移9:59:59.99。
...
因计算频谱数据是非常耗费CPU的，所以默认频谱质量设为了一般质量。你可以在[音频选项](/zh-cn/docs/working-with-audio/#options/)里改变频谱质量。如果你自己编译Aegisub且没有使用FFTW3，那么这很重要，FFTW3很快，能使默认质量提高一些。

## 卡拉OK计时

[卡拉OK计时](/zh-cn/docs/karaoke-timing-tutorial/)
